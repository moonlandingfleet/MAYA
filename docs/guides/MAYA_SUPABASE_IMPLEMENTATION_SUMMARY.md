...
Created a new `maya_supabase` directory in the `backend/api` folder with the following structure:
```
backend/api/maya_supabase/
├── __init__.py
...
3. Deploy database schema:
   Execute the contents of `backend/api/maya_supabase/schema.sql` in your Supabase SQL editor
